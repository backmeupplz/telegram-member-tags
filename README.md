...
Telegram group bot that watches group chat messages and periodically asks an OpenAI-compatible LLM provider to assign funny context-aware member tags.
...
- Uses OpenRouter free models, with fallback across configured models, for context-aware tag suggestions.
...
- `LLM_API_KEY`
- `LLM_BASE_URL`
- `LLM_MODEL` or comma-separated `LLM_MODELS`
- `DATABASE_PATH`
- `MESSAGES_PER_RETAG`
...
Legacy `FIREWORKS_API_KEY`, `FIREWORKS_BASE_URL`, `FIREWORKS_MODEL`, and comma-separated `FIREWORKS_MODELS` are still supported for existing deployments.
